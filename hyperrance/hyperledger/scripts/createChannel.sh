...
createAncorPeerTx() {
	...
	for orgmsp in InsuranceMSP HospitalMSP CustomerMSP; do
...
createChannel() {
	...
	while [ $rc -ne 0 -a $COUNTER -lt $MAX_RETRY ] ; do
		sleep $DELAY
		set -x
		peer channel create -o localhost:7050 -c $CHANNEL_NAME --ordererTLSHostnameOverride orderer.covid-ins.com -f ./channel-artifacts/${CHANNEL_NAME}.tx --outputBlock ./channel-artifacts/${CHANNEL_NAME}.block --tls --cafile $ORDERER_CA >&log.txt
		res=$?
		set +x
		let rc=$res
...
updateAnchorPeers() {
  ...
  setGlobals $ORG
    ...
    sleep $DELAY
    set -x
		peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.covid-ins.com -c $CHANNEL_NAME -f ./channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
		let rc=$res
		COUNTER=$(expr $COUNTER + 1)
...
createChannel insurance

## Join all the peers to the channel
echo "Join Insurance peers to the channel..."
joinChannel insurance
echo "Join Hospital peers to the channel..."
...
updateAnchorPeers hospital
echo "Updating anchor peers for customer..."
updateAnchorPeers customer
