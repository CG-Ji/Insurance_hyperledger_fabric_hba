...
function createOrgs() {	
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create Insurance Identities ######################"
    echo "##########################################################"
    ...
    echo "##########################################################"
    echo "############ Create Hospital Identities ######################"
    echo "##########################################################"
  ...
  echo
  echo "Generate CCP files for Insurance Hospital and Customer"
...
function network_insurance() {

  IMAGE_TAG=$IMAGETAG docker-compose -f docker/docker-compose-insurance.yaml up -d 2>&1
...
function network_customer() {

  IMAGE_TAG=$IMAGETAG docker-compose -f docker/docker-compose-customer.yaml up -d 2>&1
...
function network_hospital() {

  IMAGE_TAG=$IMAGETAG docker-compose -f docker/docker-compose-hospital.yaml up -d 2>&1
...
## call the script to join create the channel and join the peers of insurance and hospital
...
function networkDown() {
  # stop customer containers also in addition to insurance and hospital, in case we were running sample to add customer
  docker-compose -f $COMPOSE_FILE_BASE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_CA down --volumes --remove-orphans
  # (remove )docker-compose -f $COMPOSE_FILE_COUCH_ORG3 -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf organizations/fabric-ca/insurance/msp organizations/fabric-ca/insurance/tls-cert.pem organizations/fabric-ca/insurance/ca-cert.pem organizations/fabric-ca/insurance/IssuerPublicKey organizations/fabric-ca/insurance/IssuerRevocationPublicKey organizations/fabric-ca/insurance/fabric-ca-server.db
    rm -rf organizations/fabric-ca/hospital/msp organizations/fabric-ca/hospital/tls-cert.pem organizations/fabric-ca/hospital/ca-cert.pem organizations/fabric-ca/hospital/IssuerPublicKey organizations/fabric-ca/hospital/IssuerRevocationPublicKey organizations/fabric-ca/hospital/fabric-ca-server.db
...
# use this as the docker compose couch file for customer
# (remove) COMPOSE_FILE_COUCH_ORG3=addCustomer/docker/docker-compose-couch-customer.yaml
# use this as the default docker-compose yaml definition for customer
# (remove) COMPOSE_FILE_ORG3=addCustomer/docker/docker-compose-customer.yaml
#
...
if [ "$MODE" == "up" ]; then
  ...
  echo "Generate Code"
  echo
elif [ "$MODE" == "insurance" ]; then
  echo "Generate insurance"
  echo
elif [ "$MODE" == "customer" ]; then
  echo "Generate customer"
  echo
elif [ "$MODE" == "hospital" ]; then
  echo "Generate hospital"
  echo
elif [ "$MODE" == "down" ]; then
